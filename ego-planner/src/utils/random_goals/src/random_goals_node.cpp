...
void state_sub_cb(const std_msgs::Int8::ConstPtr &msg, const int &drone_id)
{
  drones_[drone_id].state = msg->data;
}
...
int main(int argc, char **argv)
{
  ...
  std::vector<ros::Subscriber> state_sub(drone_num);
  for (int i = 0; i < drone_num; ++i)
  {
    ...
    state_sub[i] = nh.subscribe<std_msgs::Int8>("/drone_" + to_string(i) + "_ego_planner_node/state", 1000, boost::bind(state_sub_cb, _1, i));
  }
  ...
  while (ros::ok())
  {
    for (int i = 0; i < drone_num; ++i)
    {
      ...
      if (!drones_[i].first_goal_sent ||
          drones_[i].state == 1 ||
          (next_random_time && (t_now - last_pub_time).toSec() > 5.0)) // 5.0s
      {
        drones_[i].state = 99;
        drones_[i].first_goal_sent = true;
        const Eigen::Vector3d reg_l = Eigen::Vector3d(-30.0, -25.0, -0.1);
        const Eigen::Vector3d reg_u = Eigen::Vector3d(30.0, 25.0, 1.6);
        // Eigen::Vector3d new_goal;
        // do
        // {
        //   new_goal << ((double)rand() / RAND_MAX) * (reg_u(0) - reg_l(0)) + reg_l(0),
        //     ((double)rand() / RAND_MAX) * (reg_u(1) - reg_l(1)) + reg_l(1),
        //     ((double)rand() / RAND_MAX) * (reg_u(2) - reg_l(2)) + reg_l(2);
        // } while ((new_goal - drones_[i].goal).norm() > 0.5);
        // drones_[i].goal = new_goal;
        drones_[i].goal << ((double)rand() / RAND_MAX) * (reg_u(0) - reg_l(0)) + reg_l(0),
            ((double)rand() / RAND_MAX) * (reg_u(1) - reg_l(1)) + reg_l(1),
            ((double)rand() / RAND_MAX) * (reg_u(2) - reg_l(2)) + reg_l(2);
        quadrotor_msgs::GoalSet msg;
        geometry_msgs::Point pt;
        msg.to_drone_ids.push_back(i);
        pt.x = drones_[i].goal(0); pt.y = drones_[i].goal(1); pt.z = drones_[i].goal(2);
        msg.goal.push_back(pt);
        msg.yaw.push_back(0.0);
        msg.look_forward = true;
        goals_pub_.publish(msg);
        ROS_INFO("time=%f, count=%d, drone_id=%d, goal=%f, %f, %f",
                 (t_now - start_time).toSec(), ++count, i, drones_[i].goal(0), drones_[i].goal(1), drones_[i].goal(2));

        last_pub_time = t_now;
        next_random_time = (double)rand() / RAND_MAX > 0.5;
      }
    }
