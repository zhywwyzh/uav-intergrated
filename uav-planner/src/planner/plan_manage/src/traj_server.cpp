...
namespace ego_planner
{
  ...
  void TrajServer::cmdFun()
  {
    ...
    if ((time_now - heartbeat_time_).toSec() > heartbeat_timeout_)
    {
      ...
      {
        do_once_ = false;
        ROS_ERROR("[traj_server] Lost heartbeat from the planner. t=%f, timeout=%f",
                  (time_now - heartbeat_time_).toSec(), heartbeat_timeout_);

        if (last_pos_.init)
          publish_cmd(last_pos_.p, Vector3d::Zero(), Vector3d::Zero(), Vector3d::Zero(), last_yaw_, 0);
        return;
      }
      // Keep executing the last received trajectory while planner is temporarily blocked.
      // This avoids truncating a valid command stream due to short heartbeat stalls.
      if (receive_traj_ && (time_now.toSec() - start_time_) > traj_duration_ + 1e-3)
      {
        receive_traj_ = false;
      }
    }
