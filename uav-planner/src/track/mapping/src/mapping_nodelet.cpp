...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  bool global_map_continuous_update_ = true;
  double global_map_x_length_ = 80.0;
  double global_map_y_length_ = 80.0;
  double global_map_z_length_ = 8.0;
  double global_map_resolution_ = 0.15;
  ...
  void map_call_back(const sensor_msgs::PointCloud2ConstPtr& msgPtr) {
    ...
    if (!global_map_continuous_update_ && map_recieved_) {
      callback_lock_.clear();
      return;
    }

    if (global_map_continuous_update_) {
      // Rebuild grid map on each callback to follow the latest streamed local map.
      gridmap_.setup(global_map_resolution_,
                     Eigen::Vector3d(global_map_x_length_, global_map_y_length_, global_map_z_length_),
                     10, true);
    ...
    if (!map_recieved_) {
      if (global_map_continuous_update_) {
        ROS_WARN("[mapping] STREAMING MAP INPUT RECEIVED. Continuous updates enabled.");
      } else {
        ROS_WARN("[mapping] GLOBAL MAP RECEIVED. Static initialization mode.");
      }
    }
    map_recieved_ = true;
    ...
    if (global_map_continuous_update_) {
      quadrotor_msgs::OccMap3d gridmap_msg;
      gridmap_msg.header.frame_id = "world";
      gridmap_msg.header.stamp = msgPtr->header.stamp.isZero() ? ros::Time::now() : msgPtr->header.stamp;
      gridmap_.to_msg(gridmap_msg);
      gridmap_inflate_pub_.publish(gridmap_msg);
    }
  ...
  void init(ros::NodeHandle& nh) {
    ...
    if (use_global_map_) {
      nh.getParam("x_length", global_map_x_length_);
      nh.getParam("y_length", global_map_y_length_);
      nh.getParam("z_length", global_map_z_length_);
      nh.getParam("resolution", global_map_resolution_);
      nh.getParam("inflate_size", inflate_size_);
      nh.param("global_map_continuous_update", global_map_continuous_update_, true);
      gridmap_.setup(global_map_resolution_,
                     Eigen::Vector3d(global_map_x_length_, global_map_y_length_, global_map_z_length_),
                     10, true);
    } else {
